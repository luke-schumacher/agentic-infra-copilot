...
class EvaluateRequest(dspy.Signature):
    ...
    suggested_agent: str = dspy.OutputField(
        desc="If redirect needed, which agent: 'governance_agent', 'telemetry_agent', or 'none'"
    )
...
class SiemensTechnicianModule(dspy.Module):
    ...
    def evaluate_incoming_request(
        ...
        return self.evaluate_request(
            ...
            agent_capabilities=(
                "Hardware fault diagnosis, equipment specifications lookup, "
                "MR-guided radiotherapy equipment expertise, technical troubleshooting, "
                "pain point analysis from healthcare institution questionnaires. "
                "Specializes in Siemens medical imaging equipment (MR scanners, LINAC, CT simulators)."
            )
        )
