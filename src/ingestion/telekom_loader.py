"""
Telekom Loader - RAG Ingestion Layer
...
Purpose:
- Ingest 'Intent' and network rules documentation
- Split into manageable chunks for RAG retrieval
- Enable queries like "What is the SLA for 5G latency?"
...
class TelekomLoader:
    """Loads and chunks Telekom PDF documentation."""
    ...
    def load(self) -> List[Document]:
        ...
        if not self.pdf_dir.exists():
            ...
            logger.warning(f"Directory created but no PDFs found. Returning empty list.")
            return []
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("Testing Telekom Loader")
    logger.info("=" * 60)
    ...
    try:
        ...
        if not documents:
            logger.warning("\nNo documents were loaded. This is expected if no PDFs exist in data/telekom/raw/")
            logger.info("\nTo test with actual data:")
            logger.info("1. Place PDF files in data/telekom/raw/")
            ...
            print(f"\n  Average chunk size: {avg_length:.0f} characters")
